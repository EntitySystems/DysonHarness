...
For bindable public types, see [api-surface.md](api-surface.md). Persistence is covered under [docs/storage](../storage/models.md). JSON dynamic toolchain: [json-dynamic-toolchain.md](json-dynamic-toolchain.md). Meta Agent mode (allowlist toolset, plan tools, child-report watch, 100K cap): [meta-agent.md](meta-agent.md).
...
- **Streaming SSE** (`stream: true`) for assistant text and optional reasoning; Completions reads `choices[0].delta.content` and `delta.reasoning_content` (+ incremental `tool_calls`, `stream_options.include_usage`); Responses handles `response.output_text.delta`, `response.reasoning_summary_text.delta` / `response.reasoning_text.delta`, function-call assembly (`output_item.added` / `function_call_arguments.delta|done` / `output_item.done`), and `error` / `response.failed`. Session consumes chunks per tool-loop round; `AssistantText` + H1 title parse only on the final no-tool round (preview stays raw until then). Per tool round, reasoning is committed into an ordered **`ReasoningLog`** (`Thought` + optional `InterimText` from non-final `reply.Content`) before previews clear; denormalized **`ReasoningText`** is the join of Thought segments only. Final assistant body stays in `AssistantText` (not InterimText). Thought/InterimText + `ReasoningText` are UI + persistence only — never injected into transcript builders. **`UserComment`** is the exception (re-emitted as user-role history; see [Turn timestamps](#turn-timestamps)). Cancel/error commits any streamed Thought when applicable, then clears previews. After each successful Completions/Responses round (main tool loop + Explore recap), `OpenAiCompatibleHttp.TryParseUsage` fills `OpenAiModelReply.Usage` and the session appends a `usage_requests` row when `IDysonUsageAnalyticsRepository` is wired (append failure is logged, not a turn error). Demo sessions and web-search / turn-summarizer Completions do not write usage rows.
- **Transient inference retry:** each Completions/Responses stream round (including missing-tool full replay and Explore budget recap) retries automatically on OpenAI stream/endpoint errors (`OpenAiCompatibleHttp.IsTransientServerError`) — **5xx**, **429**, and transport / stream-read / incomplete SSE / Responses mid-stream / invalid JSON shapes. **4xx other than 429** (400, 404, 401, 403, …) are **not** retried and fail that round immediately. Cancellations are never retried. Non-429 transients: **11 attempts** (10 retries) with fixed backoff **2s → 5s → 10s (×8)** (10s ceiling; honors cancellation; no `Retry-After`). **429** uses a separate cap: **2 retries at 10s** then fail/fallback (3 HTTP attempts; never the 10-retry schedule). Session log records the full `OpenAI API NNN …: {body}` string on every failed round (non-retryable 4xx, first retry of a still-retryable error, and exhaustion); compact countdown lines (`OpenAI transient 503 — retry 1/10 after 2s`; `OpenAI transient 429 — retry 1/2 after 10s`) remain for 5xx/429. Streaming/reasoning previews clear before the next attempt. User cancel never hops and fails the turn immediately. After those retries exhaust (and **immediately** on non-retryable 4xx including **401 / 403**), one hop per prompt to `DysonAgentSessionConfig.FallbackChatProvider` if set (OpenAI-compatible, different `SlugId`): mutate live `session.Provider` mid-turn, persist `ModelSlugId` + `ReasoningEffort`, rebuild Completions/Responses (clear `previousResponseId`, recompute API mode), and continue the round. Fallback gets its own matching budgets (11 attempts for non-429 transients; 2×10s for 429; no retry on other 4xx). If fallback also dies, fail with the fallback error (session stays on fallback). Null / empty `FallbackChatProvider` = disabled (fail the turn after retries; unlike other role pickers, empty does **not** mean “use session model”). Toast/`LastError` only after exhaustion (including fallback).
...
- **Cache-friendly requests** (`OpenAiCacheFriendlyTranscriptBuilder`):
  ...
  6. User content for **completed** history turns is `Instruction` plus any `UserComment` blocks (`USER INJECTED COMMENT:` via `FormatInjectedUserCommentsForTranscript`). The incomplete current turn does **not** splice comments onto Instruction — they go out as `currentUserPrompt` / `harnessFollowUp` after in-flight tool rounds. Rename-review mandate is appended only for the in-flight review turn.
  7. **Binary / vision attachments** (`LoadBinary`, `BrowserTakeScreenshot`, composer `UserImages`): when `DysonAgentSessionConfig.FileStorage` is set, images are uploaded once (`EnsureRemoteUrlAsync`) and the **same** presigned HTTPS `RemoteUrl` is re-emitted on every full Completions/Responses rebuild (composer images on the user message; tool images after that round’s tool ack — not hoisted onto `UserImages`). Do not re-sign a still-valid URL (signature rotation busts prompt cache). Filename lives in a text / `input_text` label — never on Completions `image_url` or Responses `input_image`. **`LoadBinary` images** and **`BrowserTakeScreenshot`** require FileStorage (`file_storage_required` tool error, no BinaryAttachment) — no data-URL fallback for new tool images. Non-image `LoadBinary` is unchanged. **`LoadBinary`** still passes through provider-native image MIME (`image/png` / `jpeg` / `gif` / `webp`); other `image/*` types Magick can decode are converted to PNG before the FileStorage gate. **Composer user images** persist on the turn (`UserImagesJson`, including local `base64Data` for thumbs) and re-emit on every rebuild. Without `RemoteUrl` (legacy / tests), tool images stay last-unanswered-round one-shot data-URL / Files. Wire shapes:

| Concern | Chat Completions | Responses |
| ------- | ---------------- | --------- |
| Image with `RemoteUrl` | `type: "image_url"`, nested `image_url: { url: https://…, detail: "auto" }` (every rebuild) | `input_image.image_url` HTTPS **string** + `detail: auto` (no `file_id`) |
| Image without `RemoteUrl` | nested data URL, last unanswered round only | Prefer Files upload `purpose=vision` → `input_image.file_id`; fallback data-URL **string** |
...
| Upload | S3 `PutObject` unsigned payload when FileStorage is set; Completions never uses OpenAI Files for vision | `EnsureRemoteUrlsForVisionAsync` then skip `OpenAiFilesClient` when `RemoteUrl` is set; otherwise Files + data-URL fallback |
...
| `Meta Agent` | Never-blocking orchestrator (page-launched; not composer-selectable). Transcript is trimmed to the newest 40 turns every 15 completed turns; context is fixed at 100K. No filesystem or shell — [meta-agent.md](meta-agent.md). |
| `Meta Agent Drone` | Implementer spawned only by a Meta Agent. Work catalog minus `AskQuestionFromParent` and `PromptUserDialogFromParent` (`TriggerParentEvent` stays), plus `ReadMetaPlan` / `SubmitMetaPlan`. Questions and section-boundary status pings use `TriggerParentEvent` (kind `message`); the parent answers with `RespondToSubagentEvent`. Normal context cascade. `StartAsyncMetaAgentDrone` requires `useWorktree`: file-mutating tasks should set true (own worktree, merge on completion); non-coding tasks (ops, testing, CI, pushes, read-and-run) should set false (parent checkout, no branch, no merge). Optional `existingWorktreePath` (false only) rebinds an already-listed checkout. A content conflict aborts the registered merge and the failed report is the parent's resolve note. |
| `Custom` | Category label; lookup uses `Config.CustomAgents` keys |

System prompts come from `DysonAgentSystemPrompts.ForMode`, then (when an `IDysonModelRepository` is available) an **available-models catalog** is appended, then an **openrules** block (`DysonOpenRules.BuildSystemPromptBlockAsync`: Root + provider-filtered AutoInclude Rules/Skills from work-root `openrules.json`, or implicit `AGENTS.md`; Paths may be local or http(s) URLs), then an optional **git worktree** suffix (`BuildWorktreePromptBlock` — pending vs bound; null when off). See [docs/openrules/README.md](../openrules/README.md) and [work-directories.md](../storage/work-directories.md)#session-git-worktrees. That models catalog is what UI / `StartSubagent.modelSlug` can select; effort tags are freeform for API `reasoning_effort` / `StartSubagent.reasoningEffort`. Mid-session mode changes use `DysonAgentSession.ApplyAgentMode` (rebuilds `Mode` / `SystemPrompt`, bumps `SystemPromptGeneration` for cache invalidation — Completions `system` / Responses `instructions` are replaced each hop); the UI host applies on prompt submit when the composer picker differs, appends a completed **`ModeSwitch`** turn before the Normal user turn (history boundary only — short harness user message in the OpenAI transcript; not omitted like `DisplayInfo`), and persists `AgentMode` + `SystemPromptSnapshot`. Work / Explore / Drone directives cover orchestrator routing, Work/Drone Wait on any Explore they start, and mandatory `SubmitSubagentReport`. Explore/Drone directives harden “report is mandatory” / “complete or impossible”, including `failed` + failure-reason summaries. Every child first turn prepends `SubagentReportRequiredMandate` (failure-reason reports are valid finishes); Explore/Drone also get mode-specific first-turn blocks (`ExploreFirstTurnReportMandate`, `DroneFirstTurnContextMandate`). Plan’s first incomplete **Plan-stint** user prompt prepends `PlanFirstTurnMandate` (Explore before finalize; transcript-only; skips `ModeSwitch` / `DisplayInfo` / `WorktreeCreating` / `PlanResult` — not raw `Turns[0]`). Security Review / Bug Review directives briefly mirror the same when used as subagents.

### Plan artifacts

In Plan mode, `SubmitPlan` (`title` + `markdown`) writes `.dyson/plans/{slug}-{sha1}.md` via `DysonFileManager`, appends a harness `PlanResult` turn (no LLM) with a turn `Instruction` that mandates updating that same `planPath` via `WriteFile` (no second `SubmitPlan` unless the user asks for a new plan), and returns `planPath` in the tool result. After `PlanResult`, the UI shows a composer Plan-ready sticky until a later `BeginBuildPlan` turn (`Host.BuildPendingPlanAsync` → folds any buffered Explore completion reports into the BeginBuildPlan Instruction, switches to Work, then `PromptBeginBuildPlanAsync`; sticky also dismisses on legacy `[BuildPlan]` user prompts). BeginBuildPlan is layout-only (Recap + Agent actions; no implementation that turn): Agent actions must prepare technical multi-Drone parallel workstreams (multitasking preferred over serial solo work; still no `StartSubagent` this turn). It mandates session todos via `CreateTodo` for each Agent actions item after `ReadFile` on the plan. After it succeeds, the host auto-enqueues a Normal continuation (`DysonBeginBuildPlanFlow.ContinuationPrompt`) that runs the implementation (preferring parallel Drone multitasking). Mutating product tools remain available (soft / prompt-only read-only); only `SubmitPlan` is hard-gated to Plan mode. Meta Agent plans are a different path: rows in the `plans` table addressed by `planId`, authored by a Meta Agent Drone via `SubmitMetaPlan` — see [meta-agent.md](meta-agent.md) and [plans.md](../storage/plans.md). `DysonMetaBuildBrief` (engine, Meta Agent `BeginBuildPlan`) is unrelated to `DysonBeginBuildPlanFlow` despite the overlapping English name.
...
| `StopSubagent` | Cancel child CTS; mark `Stopped` then `SubmitSyntheticReportAsync` (failed report through the same pipeline as a real submit; skips the agent-facing Stopped rejection). Parent sees `SubagentFailed`, not a silent stall |
...
- **Meta Agent** may spawn **Meta Agent Drone** or **Explore** only (never blocks; no `WaitForSubagent` in the catalog). **Meta Agent Drone** may spawn **Explore** or classic **Drone**; no nested Meta Agent Drone. Meta Agent itself is page-launched only.
...
Soft spawn / restore / inter-agent event coverage: `Harness.Tests` (`DysonSubagentSpawnGateTests`, `DysonSubagentRestoreTests`, `DysonParentEventTests`, `DysonPromptUserDialogTests`, `DysonChildReportWatchTests`, `DysonSyntheticReportTests`) — `dotnet test src/Harness/Harness.Tests/Harness.Tests.csproj`. Return shape: `DysonStartSubagentResult` (`subagentId`, `persistenceId`, `agentMode`, `title`, optional `modelSlug` / `modelLabel`). Provider/exception kickoff failures synthesize a failed report through `SubmitSyntheticReportAsync` (PromptAsync error → last turn snippet → harness message; exceptions as `{Type}: {Message}`). A later successful `SubmitSubagentReport` can supersede that harness Failed (see tool row above).

**Child-report watch (all parents, not Meta-Agent-only):** a child turn that ends idle without `SubmitSubagentReport` and with nothing queued gets a `ChildReportReminder` turn on the child (max 2, kind 19). After the reminders, or when the child goes terminal without a report (`StopSubagent` / `StopAllExecution`, crash, `Interrupted` after process restart), the harness calls `SubmitSyntheticReportAsync` — a failed `SubmitSubagentReport` that skips the `Stopped`/`Interrupted` rejection (that guard exists to stop the *agent* retrying after a stop, not to stop the harness closing the loop) and nothing else. Completed / already-Failed still reject, which is the idempotency bookkeeping. `remindersSent` and `LastReportSummary` are per-process (in-memory); restart recovery cannot read them.

**Stop vs Failed:** a user halt (`StopAllExecution`) and `StopSubagent` leave the child **`Stopped`**. The synthetic path records a report and notifies the parent with `SubagentFailed`, but `TryAcceptSubagentReport` **keeps** `Stopped`/`Interrupted` — a synthesized `SubagentFailed` means "this child is done and will not report", not "the work failed". Cards and `ListMetaAgentDrones` read `Status`. An agent's own second `SubmitSubagentReport` still rejects. Idle-without-report after two reminders does set `Failed` (the child was still `Active`). Full detail: [meta-agent.md](meta-agent.md)#child-report-watch-all-parents.

`DysonSessionRecoveryService` has no live graph: it marks Active descendants `Interrupted` and appends one parent `Interrupt` log (`SubagentFailed`, summary `application restart`). It does **not** call `SubmitSyntheticReportAsync`. Idempotent if that log already exists.
...
`DysonPluginMcpResolver` and `DysonPluginMcpHost` provide a default-deny package-owned MCP runtime seam. Installation enablement alone never starts a server. A caller must supply an explicit per-installation/server executable or network grant. Tools use `plugin__{pluginId}__{serverId}__{toolName}` names and reject deterministic namespace collisions. Session create/resume assigns `DysonPluginMcpHost` and refreshes it on the thread pool. Create/resume does not wait for stdio/HTTP initialize. Tools merge into the live `McpPipeline` and bump `SystemPromptGeneration` when connect finishes. A server that misses the 30s handshake is `Error` on the host, not a UI stall.
...
| `ListLongRunningShells` | Compact roster for the **session cwd** (`id`, `status`, `shell` = configured name, short `command`, `exitCode`, `startedUtc`). Registry buckets stay `WorkDirectoryId`; `List(id, workingDirectory)` / MCP list filter by `NativeRootPath` so a worktree session does not see main-tree shells |
...
`DysonAgentTurn` carries **`StartedUtc`** (set on live turn create; restored from `CreatedUtc`) and **`CompletedUtc`** (set when the host persists turn completion; null while streaming). Ordered **`ReasoningLog`** (`Thought` / `InterimText` per tool round, plus **`UserComment`**) plus denormalized **`ReasoningText`** (Thought join) hold provider thinking and mid-turn user steers for UI + DB reload. **Thought/InterimText stay UI+DB-only** — never injected into model transcripts. **`UserComment` is the exception:** transcript builders re-emit each segment as user-role history (`USER INJECTED COMMENT: {text}`) on **completed** turns, appended after Instruction on the `[turnId=…]` user message (`DysonContextOptimizer` never rewrites that message). Compact tool-history (`ToolHistoryOptimized`) still emits the instruction user message first, so comments survive `[compact]`. `SummarizeTurns` stubs (`FormatSummaryStub`) append the same blocks after `ContextSummary` so the LLM summary cannot drop them. `DropTurnContext` still drops the whole turn (comments included). Restore synthesizes one Thought from legacy `ReasoningText` when the log is empty.

In-flight: `InjectTurnComment` succeeds only when `turnId` is on `_inFlightPromptStack`. The OpenAI tool loop drains `HasPendingUserComments` at the start of each round into `harnessFollowUp`. A no-tool reply while comments arrived mid-stream commits thought only and continues — it does not set `AssistantText`. `EndsCurrentTurn` is not hijacked (`CompleteTask` / `ExpandThoughtProcess` / … still soft-close); the user can `ContinueWork` on the confirm turn. Comments already in thinking history stay visible, and the **next** turn still sees them via history emission even if they never drained into `harnessFollowUp`. The in-memory drain queue is not persisted (process restart interrupts in-flight turns); `UserComment` segments persist via `ReasoningLogJson`. UI timestamps are transcript chrome only — not injected into model messages. Display format in the UI: local wall clock `dd/MM/yyyy HH:mm`.
...
### Meta Agent maintenance tick

After a completed turn is persisted, runtime and host persist paths call `ApplyMetaMaintenanceTickAsync` (`DysonMetaMaintenanceTick`). **Meta Agent only.** Every **15** completed turns (in-memory `TurnsSinceMetaMaintenance`; `MetaMaintenance` turns do not increment it): hard-delete completed turns older than the newest **40** (`DeleteTurnsAsync` + in-memory `Turns`) and enqueue exactly one `MetaMaintenance` turn at the tail asking the agent to prune finished agents (≤20) / stale plans / todos. A per-call "too many agents" notice on `ListMetaAgentDrones` would change the prompt prefix on every dispatch and destroy prompt caching; one periodic tail turn leaves the cached prefix intact. Evicting per turn past 40 would miss the cache continuously. `MaxTurns` is the floor after a tick (~40–54 live). In-flight, incomplete, newest `FullSummarize`, and unprocessed `MetaMaintenance` turns are never deleted. Independent of DropContext (token guard) and CompactConversation (`FullSummarize`). Covered by `DysonMetaMaintenanceTickTests` / `DysonMetaAgentContextCapTests`. Toolset and plan tools: [meta-agent.md](meta-agent.md).
