...
| `Meta Agent Drone` | Implementer spawned only by a Meta Agent. Work catalog minus the `*FromParent` trio, plus `ReadMetaPlan` / `SubmitMetaPlan`. Normal context cascade. As built, children still inherit the parent's worktree columns (no per-drone checkout yet). |
