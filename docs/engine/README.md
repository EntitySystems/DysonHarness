...
- **Snippet contract:** Python `command` is a `-c` snippet; Node `command` is a `-e` snippet. When the session enum includes `Python` and/or `Node` (`OrdinalIgnoreCase`), `ShellExecute` and `StartLongRunningShell` descriptions append that sentence — both: “Python command is a snippet (`-c`); Node command is a snippet (`-e`).”; Python only / Node only use the matching half. Plan warning still appends after that sentence. No `DysonShellType` values added.
