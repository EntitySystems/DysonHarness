...
- **Cache-friendly requests** (`OpenAiCacheFriendlyTranscriptBuilder`):
  ...
  5. Completions always sends full local `messages[]`. Responses always uses `store: true` so tool-loop chaining works. `previous_response_id` is omitted on the first full rebuild; required on delta tool hops; passed on mid-loop full rebuilds when known (cleared before child-report-gate nudge retries). Full rebuilds keep prefix/`prompt_cache_key` caching. Within a tool loop, delta hops send `previous_response_id` + only new `function_call_output` items.
