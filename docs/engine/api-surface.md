...
| `DysonWorkspaceToolExecutor` | Workdir-scoped file tools + `RenameSession` + `GetDateTime` + `SubmitPlan` (Plan mode only → `.dyson/plans/` + PlanResult turn) + `ShellExecute` + web search/fetch tools (tool-owned summarize) + **subagent tools** (`StartSubagent` / `ListSubagents` / `WaitForSubagent` / `InspectSubagentLog` / `StopSubagent` / `SubmitSubagentReport`) + **inter-agent / Ask** (`TriggerParentEvent` / `RespondToSubagentEvent` / `TriggerSubagentEvent` / `AskQuestion` / `AskQuestionFromParent`) + **session todo tools** (`ListTodos` / `CreateTodo` / `UpdateTodo` / `DeleteTodo`); stubs for the rest |
...
| `DysonBeginBuildPlanFlow` | Factory + Recap / Agent-actions Instruction for composer Build plan (`PromptBeginBuildPlanAsync`); optional Explore report blocks folded in from Plan-mode buffer; `ContinuationPrompt` + `ShouldEnqueueBuildContinuation` (host enqueues a Normal turn after successful BeginBuildPlan) |
...
| `DysonFileManagerSelfCheck` / `DysonPlanResultSelfCheck` | Assert-only naming/sandbox + PlanResult / BeginBuildPlan (+ report injection / Plan deferral / continuation enqueue) + `ApplyAgentMode` / Plan-ready checks (UI `Program` startup) |
| `DysonShellExecutePlanWarningSelfCheck` | Assert-only Plan ShellExecute description/preamble soft warning (UI `Program` startup) |
