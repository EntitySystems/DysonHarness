...
| `DysonToolCallResult` | Completed/failed payload (`IsError`, `Content`, optional `BinaryAttachment`, `EndsCurrentTurn` soft-closes the calling turn after the staged round) |
...
Assert coverage for spawn gates, parent events, session todos, PlanResult, rethink, ExpandThoughtProcess / DropTurnContext, shells, Grep/LoadBinary, SSRF/search parsers, and related helpers lives in `Harness.Tests` (`dotnet test src/Harness/Harness.Tests/Harness.Tests.csproj`).
...
| `DysonExpandThoughtProcess` | Expand-thought turn factory; Instruction includes optional `DropTurnContext` hygiene; `ContinuationPrompt` + `ShouldEnqueueContinuation` (host Normal follow-up) |
