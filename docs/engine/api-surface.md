...
- `DysonWorkDirectoryEntity` (`SubjectId`; unique `(SubjectId, AbsolutePath)`; nullable `GitOrigin` / `GitProvider` slugs), `DysonWorkDirectoryConfigurationEntity` (`ConfigJson` TEXT / `JsonNode`; `mcpActive` default true), `DysonWorkDirectoryConfig` helpers, `DysonNativeFolderPicker`, `DysonGitInfo`, `DysonGitProvider`, `DysonWorkDirectoryService` (refresh origin on activation; `GetGitProvider` maps stored slugs including `cursor-origin`)
