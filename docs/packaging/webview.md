...
- **WebGPU + ANGLE D3D11 + GPU compositing:** `DysonCefStaHost.InitializeCef` sets Chromium switches `enable-unsafe-webgpu`, `ignore-gpu-blocklist`, and `use-angle=d3d11` before `Cef.Initialize` (shared by the desktop shell and agent browser windows). It also **removes** `disable-gpu-compositing`, which `CefSharp.Wpf.CefSettings` adds by default for OSR resize ([CefSharp#4953](https://github.com/cefsharp/CefSharp/issues/4953)). With that flag left on, HwndHost agent tabs still get a WebGPU adapter/device (and DOM HUD) but canvas layers stay black / FPS stalls — GPU compositing is required to present WebGPU swapchain textures into the windowed HWND. Do **not** force `use-angle=d3d12`: ANGLE D3D12 leaves no `SharedImageBackingFactory` for `WebgpuSwapChainTexture` / `WebGPUSwapBufferProvider`, so canvas `getContext('webgpu')` configure/present fails and the device is destroyed after adapter init. Explicit ANGLE **D3D11** registers the D3D SharedImage factory needed for swapchain present. `enable-gpu-rasterization` is intentionally omitted (earlier crash correlate). CefSharp **149** NuGet already ships `dxil.dll` / `dxcompiler.dll` (required for Dawn WebGPU on win-x64; see [CefSharp discussion #4662](https://github.com/cefsharp/CefSharp/discussions/4662) / [CEF #3646](https://github.com/chromiumembedded/cef/issues/3646)). `DysonCefStaHost` fails fast if either DLL is missing beside the exe. **#4662's failure mode was "No WebGPU adapters found"** (missing DXC) — not a black canvas after `WebGPURenderer ready`. Host exes also embed `Harness.WindowsBrowser/app.manifest` (Windows 10+ compat + DPI) as required by the CefSharp NuGet Readme / #4662 for GPU detection. For Zafirn-style pages, also remember the character-select `#arena-overlay` is nearly opaque until Join succeeds — that can look like a "black WebGPU canvas" even when the swapchain is presenting. **Agent tabs** (games/WebGPU pages) use `CefSharp.Wpf.HwndHost.ChromiumWebBrowser` with `ActivateBrowserOnCreation` and a layout nudge when the HWND would otherwise be 0×0 — this path is shared by **both** `Harness.UI` (web host on `:5180`) and `DysonHarness.UI.Windows` when opening agent windows. The **desktop shell** Blazor view stays on OSR `CefSharp.Wpf.ChromiumWebBrowser` with `BrowserSettings.WindowlessFrameRate = 60` and **cannot** host external game origins in-process (those open in agent windows / OS browser); enabling GPU compositing for agents may reintroduce OSR resize glitches on that shell view. Flags only apply on a **fresh** `Cef.Initialize` — kill `Harness.UI` / `DysonHarness` / `CefSharp.BrowserSubprocess` before restart. Verify: in an **agent** tab open `chrome://gpu` (WebGPU hardware; ANGLE backend D3D11; Compositing hardware) and/or canvas WebGPU configure+present; shell also allows `chrome://` (see `ExternalNavigationHandlers`). Automated gate: `dotnet test src/Harness/Harness.Tests --filter DysonCefGpuPresentTests` (`WebGpu_WebGl_And_NonBlack_Present` — WebGPU **canvas** configure+present, WebGL clear, non-black CDP present; adapter-only is insufficient). Needs a D3D-capable GPU and the VC++ 2022 x64 redistributable; if swapchain/SharedImage fails, check `%LocalAppData%\DysonHarness\cef-debug.log` for `WebGPUSwapBufferProvider`. Snip over agent tabs uses a CDP screenshot overlay because HWND airspace blocks live WPF rubber-bands (see **Snip → agent composer**).
