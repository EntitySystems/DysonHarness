...
- `image_generation_model_slug_id` — Guid string of the dedicated model for `GenerateImage`. It is resolved only when the selected slug is enabled and its provider is credentialed, unmanaged direct OpenAI at `https://api.openai.com/v1`; empty, malformed, missing, disabled, or unsupported values leave image generation disabled. This has no reasoning-effort companion and never falls back to the session chat model. Edited via Settings → Agent behavior; hydrated onto `DysonAgentSessionConfig.ImageGenerationProvider` at session create/resume.
...
- `meta_agent_drone_model_slug_id` — Guid string of the default model slug for Meta Agent Drone subagents when the spawning tool omits `modelSlug` (not `StartSubagent`; that tool is not in the meta catalog); empty / missing ⇒ inherit parent session model. Edited via Settings → Agent behavior; hydrated onto `DysonAgentSessionConfig.MetaAgentDroneDefaultProvider` at session create/resume.
- `meta_agent_drone_reasoning_effort` — optional reasoning-effort override for that default slug; empty / missing ⇒ the slug’s `DefaultReasoningEffort`. Cleared when the companion slug changes. Hydrate passes `null` (not `""`) for Default so `OpenAiCompatibleAgentProvider` still applies `slug.DefaultReasoningEffort`.
...
- `ui_accent` — `"blue"` / `"green"` / `"red"` / `"purple"` / `"amber"`; Settings → General accent. Same persistence as `ui_theme`. Missing ⇒ one-shot migrate from `localStorage`, then blue. Amber is honey (`#e09b58`), not a high-chroma orange.
...
Indexes: `(SubjectId, OccurredUtc)`, `(SubjectId, WorkDirectoryName)`, `(SubjectId, RootSessionId)`. `AppendAsync` stamps `SubjectId` from `IDysonSubjectContext`. `ListAsync(workDirectoryName?, fromUtc?, toUtc?, cancellationToken?, modelSlug?)` is newest-first; the trailing optional `modelSlug` preserves positional callers while allowing SQLite to apply the subject, work-directory, model, and inclusive UTC-window filters together. `ListByRootSessionAsync` feeds the Usage-rail recap (`DysonUsageSessionRecap.FromRows` nests Total → model → effort). Migration `AddUsageRequests`. `AddDysonLocalDb` registers `IDysonUsageAnalyticsRepository`.

Usage charts use Engine's pure `DysonUsageTimeSeries`: `FromRows(rows, fromUtc, toUtc, bucket)` emits every UTC-aligned hourly or daily bucket intersecting the inclusive window, including zero-valued points, and sums only `InputTokensAfterCache`, `CacheTokens`, and `WriteTokensAfterCache`. `ResolveBucket` selects hourly for 1-day and custom windows up to two days, and daily for 7-day, 30-day, and longer custom windows. The chart host obtains `GetFilterOptions(unfilteredWindowRows)` **before** work-directory/model filtering so selectors retain all distinct work-directory names and each model's slug plus latest display-alias snapshot.
...
| `ManagedSource` | Optional managed-provider type tag: CLIProxy sources use `cliproxy-*`; direct managed uses `openrouter` or `orcarouter`. Unique when non-null within scope. Null = user-owned manual provider |
| `CreatedUtc`, `UpdatedUtc` | `DateTime` UTC |
...
Managed rows cover two distinct lifecycles:

- **Direct managed (OpenRouter / OrcaRouter):** `ManagedSource=openrouter` or `orcarouter`, `ProviderKind=OpenAICompatible`, `OpenAiApiMode=Completions`, Completions base URL on `BaseUrl`, and the user-supplied Bearer API key on `ApiKey`. Import starts with no slugs. The Browse models modal reads the live catalog and persists only models the user enables; unchecking or card **Remove** deletes the slug row. Disable-without-delete remains CLIProxy-only. API keys remain plaintext-local in SQLite—there is no OS keychain in this change.
- **Managed CLIProxy:** Settings → Models can import ChatGPT Codex, Grok Build, Antigravity, Kimi, Claude Code, or Meta Muse through the pinned local [CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI) binary under `{AppContext.BaseDirectory}/external/cliproxy/{version}/` (lazy download; not LocalAppData). These `cliproxy-*` rows use `OpenAiApiMode=Responses`, `BaseUrl=http://127.0.0.1:{port}/v1`, and the local proxy Bearer key. OAuth goes through the CLIProxy Management API; **Verify** syncs `/v1/models`. Claude uses the proxy’s OpenAI/Responses surface, not Anthropic Messages. Meta Muse uses the same device-code management path as Grok (`meta-auth-url`).

`IDysonModelRepository.UpsertManagedProviderAsync(..., bool shared = false, bool syncSlugs = true, ...)` keeps an id-stable row per `ManagedSource`. With the default `syncSlugs: true`, used by CLIProxy Verify, it performs the full-catalog merge: existing rows keep `Id`, `IsEnabled`, and `DefaultReasoningEffort` while catalog fields refresh; new API models insert enabled with catalog default effort; missing API models are removed. Direct-managed API-key rotation calls `UpsertManagedProviderAsync(..., syncSlugs: false)`, which updates provider fields without merging or deleting any slug rows.

`UpsertManagedSlugAsync(Guid providerId, ManagedSlugSpec spec, bool enabled, ...)` enables a model selected from a direct managed catalog. It inserts or updates that managed slug by name, preserving its row id and user default effort while refreshing catalog alias/modes and setting `IsEnabled`. Browse uncheck and card **Remove** call `RemoveSlugAsync`, which deletes direct-managed (`openrouter` / `orcarouter`) slugs and still rejects `cliproxy-*`. `SetSlugEnabledAsync(false)` remains the CLIProxy disable-without-delete path. `UpdateProviderAsync` and ordinary slug add/update reject when `ManagedSource` is set; managed-specific setters remain the mutation path.
...
| `IsEnabled` | When false, omitted from new selection catalogs (picker, Composer `/model`, system-prompt catalog, `FindSlugByNameAsync`); Settings → Models still lists it. CLIProxy managed slugs expose Enable/Disable; direct-managed slugs use Remove (delete) instead; manual/custom slugs stay always selectable (API rejects toggle). Default `true` (migration + new inserts). |
| `DefaultReasoningEffort` | Optional freeform default effort (e.g. `high` / `low`); null/empty = omit. Completions normally uses top-level `reasoning_effort`, but OpenRouter Completions uses nested `reasoning.effort`; OrcaRouter Completions stays on top-level `reasoning_effort`; Responses also uses nested `reasoning.effort` |
...
- **Providers:** list (include slugs), get, create (`shared` flag), update (incl. `ApiKey` / `BaseUrl` / `OpenAiApiMode`; rejected when `ManagedSource` set; `shared` flag), `UpsertManagedProviderAsync` (id-stable by source; `syncSlugs: true` full-catalog merge by default, `false` updates provider fields only; `shared` flag), delete
- **Slugs:** add under a provider (optional `defaultReasoningEffort` + `reasoningModes`; rejected when provider is managed), update (alias / slug / default effort / modes / is-default; rejected when managed), remove (allowed for direct-managed `openrouter` / `orcarouter`; rejected for CLIProxy), `UpsertManagedSlugAsync` (managed only; enable/refresh one catalog choice), `SetSlugEnabledAsync` (managed only; CLIProxy disable-without-delete), `SetSlugDefaultReasoningEffortAsync` (managed only; blank → omit), `SetSlugDefaultMaxTargetContextTokensAsync` (managed only; null → harness 100K)
...
6. When non-empty, Completions request bodies normally include top-level `"reasoning_effort": "<value>"`; OpenRouter Completions and all Responses requests use nested `"reasoning": { "effort": "<value>" }`. OrcaRouter Completions stays on top-level `reasoning_effort`. Blank/null omits the field.
