...
`DysonPlanEntity` is the EF row. Every row written today is `Kind = MetaPlan`. The UI plans column (`MetaPlanList`) reads through `ListAsync` (read-only: no Build button, no click-to-open). Engine writes: Meta Agent Drone `SubmitMetaPlan`; Meta Agent `SetPlanStatus` / `BeginBuildPlan` / `DeletePlan`. See [meta-agent.md](../engine/meta-agent.md).
