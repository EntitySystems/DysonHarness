...
| `Components/Chat/` | `ChatPanel`, `SessionTodoOverview`, `SessionSubagentOverview`, `TurnBlock`, `PlanResultBlock`, `PlanReadyPopover`, `SubagentCard`, `SubagentEventBlock`, `AskQuestionPopover`, `Composer`, `ComposerHelpModal` (`/help`), `AgentModePicker` |
...
| `Composer` | Unified `.composer-shell` (context chips + textarea + footer toolbar/Send); muted Ctrl+Enter / paste-drop-images / **`/help for commands`** hint under the shell. Pending skills (`Host.PendingSkillNames`) and pending images (`Host.PendingImages`) render as dismissible chips/thumbnails above the textarea; attach via **+** file picker (`image/*`), Ctrl+V paste, drag-drop onto the shell, or **browser Snip** (`SnipCaptured` → `QueuePendingImage`; optional `HtmlRef` empty today) (JPEG-compressed via `DysonUserImageFactory`, max 8). Send enabled when text **or** pending images exist; images attach onto the Normal turn as `UserImages`. Left-aligned footer toolbar (+ attach, model chip, compact **Effort** `<select>` of the selected slug’s `ReasoningModes` plus **None** (null → omit; legacy current value kept as an extra option if not in the list), mode, git branch chip) with Send right-aligned. **`OpenModelPicker()`** opens the `ModelSlugPicker` modal (empty-state Switch model CTA). **`OpenSkillSearch()`** opens `SkillSearchModal` (empty-state Browse skills CTA / `/skill-search`). **`OpenHelp()`** / slash **`/help`** opens `ComposerHelpModal` (scrollable built-in + pattern templates; Insert fills the composer). **AskQuestionPopover** / **PlanReadyPopover** stay above the shell when set. Typing `/` as the whole prompt token opens a dense slash-command overlay above the textarea (`/ask` `/plan` `/work` modes, `/new` → `OnNewSession` / `StartNewAsync`, **`/skill-search`** → `SkillSearchModal` (matched before `/skill-` local catalog filter), **`/help`** → `ComposerHelpModal`, `/skill-` catalog search (included + `.dyson/skills` + openrules AgentOptional — see [docs/openrules](../openrules/README.md)) → queues pending skill attach with `loadIndexOnly: true` on next non-empty prompt, `/[model]` fuzzy match on **enabled** slug/alias like the model picker — applies to the live session via `SetSessionModelSlugAsync` when focused; max 5; ↑↓ Enter Esc; send strips+applies a leading command; `/skill-… remainder` attaches to that turn). Logic in `ComposerSlashCommands` (Facts in `Harness.Tests`) |
...
- **Install package (per provider)** → `{activeWorkRoot}/.dyson/skills/{safeSlug}/` (slugs with `/` become filesystem-safe via `/` → `-`; overwrite same slug; strip a single top-level zip folder when present so `SKILL.md` sits at the skill root):
  ...
  - **ClawHub** — site `GET /api/v1/download?slug=`; ZIP extracts like Skills Directory, or JSON `public-github` handoff → fetch that GitHub zip and extract. Respects `429` / `Retry-After` with a clear error (no aggressive retries).
  - **Skills Directory** — `GET https://www.skillsdirectory.com/api/skills/{slug}/download` → zip → extract.
  Installed skills are then available to `LoadSkill` / `/skill-` like any other `.dyson/skills` entry — see [work-directories](../storage/work-directories.md).
