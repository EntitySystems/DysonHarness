...
- **Auto-turn on report:** on parent `SubagentCompleted` / `SubagentFailed` / `SubagentStopped` interrupt, enqueue a harness report prompt for that parent; when parent `!IsBusy` and **not** in Plan mode, FIFO `PromptAsync` with the report summary under a bold **Report** label (does not cancel in-flight parent work). Kickoff failures also surface a concrete reason here. In **Plan** mode, completions are buffered (UI still updates); **Build plan** folds them into the BeginBuildPlan Instruction (one `#` title + bold labels; reply still mandates `` ## Recap `` / `` ## Agent actions ``), or leaving Plan without Build drains them as Normal auto-turns. `SubagentEvent` / Ask UI are not deferred.
...
- **Plan-ready sticky:** `PendingPlanReady` / `BuildPendingPlanAsync` — after latest `PlanResult` with a path, composer shows View / Build until a later `BeginBuildPlan` turn (assistant must emit Recap + Agent actions; buffered Explore reports are injected into that Instruction); legacy `[BuildPlan]` user turns still dismiss sticky. After a successful BeginBuildPlan, `PromptOnSessionAsync` enqueues `DysonBeginBuildPlanFlow.ContinuationPrompt` as a Normal turn (drained by the existing prompt queue) so Work continues without a manual message
