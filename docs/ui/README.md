...
| `Components/Chat/` | `ChatPanel`, `SessionTodoOverview`, `SessionSubagentOverview`, `TurnBlock`, `PlanResultBlock`, `SubagentCard`, `SubagentEventBlock`, `AskQuestionPopover`, `Composer`, `AgentModePicker` |
...
| `PlanResultBlock` | Plan title + relative path + **Open plan** → `Host.OpenFileViewerAsync` |
| `FileViewerOverlay` | Fixed backdrop overlay on `Home` (Escape / ×); markdown via `MarkdownRenderer` or monospace code; click-to-highlight Comment chrome is UI-only (not persisted) |
